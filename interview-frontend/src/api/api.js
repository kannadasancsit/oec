...
export const addAssignUserToProcedure = async (planId, procedureId,userId) => {
    const url = `${api_url}/Plan/AddUsersProcedureToPlan`;
...
export const getPlanProceduresUsers = async (planId) => {
    const url = `${api_url}/PlanProcedureUsers?$filter=planId eq ${planId}&$expand=procedure`;
    const response = await fetch(url, {
        method: "GET",
    });
