...
const Plan = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      setPlanProceduresUsers(planProceduresUsers);
    })();
  }, [id]);
  ...
  const handleAssignUserToProcedure = async (userId,procedure) => {
    const hasProcedureInPlanUsers = planProceduresUsers.some((p) => p.userId === userId && p.procedureId===procedure.procedureId);
    if (hasProcedureInPlanUsers) return;

    await addAssignUserToProcedure(id, procedure.procedureId,userId);
    setPlanProceduresUsers((prevState) => {
      return [
        ...prevState, 
        {
          planId: id,
                    ...
                    <div>
                      {planProcedures.map((p) => (
                        <PlanProcedureItem
                          ...
                          planProceduresUsers={planProceduresUsers}
                        />
                      ))}
                    </div>
                  </div>
